...
describe("TopNewEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:TopNew(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:TopNew(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set TOP_NEWS_TEST_TOP_NEW_ENTID JSON to run live")
      return
    end
...
function top_new_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("TOP_NEWS_TEST_TOP_NEW_ENTID")
  ...
  if env["TOP_NEWS_TEST_LIVE"] == "TRUE" then
