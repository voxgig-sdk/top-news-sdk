...
local TopNewEntity = {}
TopNewEntity.__index = TopNewEntity


function TopNewEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TopNewEntity)
  self._name = "top_new"
  self._client = client
...
function TopNewEntity:get_name()
  return self._name
end


function TopNewEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TopNewEntity.new(self._client, opts)
end
...
function TopNewEntity:mark_deleted()
  self._deleted = true
end


function TopNewEntity:deleted()
  return true == self._deleted
end


function TopNewEntity:data_set(args)
  if args ~= nil then
...
function TopNewEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TopNewEntity:match_set(args)
  if args ~= nil then
...
function TopNewEntity:match_get()
...
function TopNewEntity:stream(action, args, callopts)
...
function TopNewEntity:_run_op(ctx, post_done)
  local utility = self._utility
