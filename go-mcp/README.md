...
[MCP](https://modelcontextprotocol.io) server exposing the TopNews SDK as
two agent tools — `top-news_list` and `top-news_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/top-news-mcp)
make build

# 2. Provide credentials via the environment
export TOP_NEWS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user top-news \
  -- /absolute/path/to/top-news-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./top-news-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// top-news_list: first page of records
{ "entity": "top_new" }
{ "entity": "top_new", "query": { } }

// top-news_load: one record by id
{ "entity": "top_new", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/top-news-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user top-news \
     -- "$PWD"/dist/*/top-news-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `top-news_list` and `top-news_load` tools now appear
   in new sessions. Ask the agent to *"list top_new using top-news"*
   and it calls `top-news_list` with `{"entity":"top_new"}`.
...
```sh
export TOP_NEWS_APIKEY=sk_live_xxx            # API key
export TOP_NEWS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `top-news_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `top-news_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `TOP_NEWS_APIKEY` | API key sent with every request. |
| `TOP_NEWS_BASE` | Optional override of the API base URL. |
...
```sh
./top-news-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"top-news_load","arguments":{"entity":"top_new","query":{"id":1}}}}'
```
