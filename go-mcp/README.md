...
MCP server exposing the TopNews SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `top-news_list` | `entity`, optional `query` map | First page of records as JSON |
| `top-news_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `top-news_list` and
`top-news_load` tools become available in new sessions.
