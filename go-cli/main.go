// Command TopNews-cli is a CLI + REPL for the TopNews SDK,
...
const prompt = "top-news"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "top_new"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      TOP_NEWS_APIKEY (api key), TOP_NEWS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: TOP_NEWS_APIKEY carries the API key and
	// TOP_NEWS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("TOP_NEWS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("TOP_NEWS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTopNewsSDK(opts)
