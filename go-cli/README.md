...
boru-driven command-line client **and** interactive REPL for the TopNews
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/top-news-cli)
make build
...
./top-news-cli list top_new

# 5. Override the API base URL for a single call
TOP_NEWS_BASE=https://api.example.com ./top-news-cli list top_new

# 6. No arguments -> interactive REPL
./top-news-cli
top-news> list top_new
top-news> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/top-news-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/top-news-cli list top_new
   ```
...
Run with no arguments to open a REPL (prompt `top-news>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `top_new`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/top-news-cli`. |
...
(`./top-news-cli <expr>`) and interactively (the REPL), and expressions compose the
