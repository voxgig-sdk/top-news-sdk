...
import (
	...
	omni "github.com/voxgig-sdk/top-news-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TopNewsError); is {
				out := map[string]any{
					"name":    "TopNewsError",
...
func omniCtx(arg any, client *sdk.TopNewsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TopNewsSDK); is {
			client = live
