...
func TestTopNewEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.TopNew(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.TopNew(nil).Stream("list", nil, nil) {
...
func top_newBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("TOP_NEWS_TEST_TOP_NEW_ENTID")
