...
class TopNewEntity
  ...
  def make
    opts = @_entopts.dup
    TopNewEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = TopNewsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [TopNew, Hash] the current TopNew data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = TopNewsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of TopNew fields)
  def match_get
  ...
  #
  # @param reqmatch [TopNewListMatch, Hash, nil] match filter (any subset of
  #   TopNew fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<TopNew>, Array] the matching TopNew items; raises TopNewsError on failure
