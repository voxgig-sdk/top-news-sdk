...
class TopNewDirectTest < Minitest::Test
  def test_direct_list_top_new
    setup = top_new_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-top_new", setup[:live] ? "live" : "unit")
    if _should_skip
...
def top_new_direct_setup(mockres)
