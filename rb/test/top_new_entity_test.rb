...
class TopNewEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = TopNewsSDK.test(seed, nil)
    seen = base.TopNew(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.TopNew(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set TOP_NEWS_TEST_TOP_NEW_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def top_new_basic_setup(extra)
  ...
  entid_env_raw = ENV["TOP_NEWS_TEST_TOP_NEW_ENTID"]
