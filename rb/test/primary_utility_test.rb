...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = TopNewsOmni.make_runner(TEST_JSON_FILE, TopNewsSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : TopNewsOmni.errify(err)
    end
