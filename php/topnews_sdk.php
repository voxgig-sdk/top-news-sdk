...
class TopNewsSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = TopNewsHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = TopNewsHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!TopNewsFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, TopNewsFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return TopNewsUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = TopNewsHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = TopNewsHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = TopNewsHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new TopNewsError($op . "_allow",
                "TopNewsSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = TopNewsHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new TopNewsError("graphql_error",
                "TopNewsSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function TopNew($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_top_new === null) {
                $this->_top_new = new TopNewEntity($this, null);
            }
            return $this->_top_new;
        }
        return new TopNewEntity($this, $data);
    }
