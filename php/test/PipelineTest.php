...
{
    private static function utility(): TopNewsUtility
    {
        return new TopNewsUtility();
    }
    ...
    private static function ctx(array $over = []): TopNewsContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TopNewsContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TopNewsOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TopNewsError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TopNewsOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TopNewsMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TopNewsOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TopNewsMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TopNewsOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TopNewsMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TopNewsSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TopNewsMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            ...
            'result' => new TopNewsResult([]),
        ]);
        [, $err] = TopNewsMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new TopNewsSpec([]),
            'response' => null,
            'result' => new TopNewsResult([]),
        ]);
        [, $err] = TopNewsMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TopNewsSpec(['step' => 's']),
            'response' => new TopNewsResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TopNewsResult([]),
        ]);
        [, $err] = TopNewsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TopNewsSpec(['step' => 's']),
            'response' => new TopNewsResponse(self::resp(200, ['v' => 1])),
            'result' => new TopNewsResult([]),
        ]);
        [, $err] = TopNewsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TopNewsSpec(['step' => 's']),
            'response' => new TopNewsResponse(self::resp(200, ['v' => 2])),
            'result' => new TopNewsResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TopNewsMakeResponse::call($ctx);
    ...
    {
        $preset = new TopNewsResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TopNewsSpec([]),
            'response' => new TopNewsResponse([]),
            'result' => new TopNewsResult([]),
        ...
        [$got, $err] = TopNewsMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TopNewsResult([])]);
        [, $err] = TopNewsMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TopNewsSpec([]), 'result' => null]);
        [, $err] = TopNewsMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TopNewsSpec(['step' => 's']),
            'result' => new TopNewsResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TopNewsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TopNewsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TopNewsSpec(['step' => 's']),
            'result' => new TopNewsResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TopNewsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TopNewsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TopNewsResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TopNewsSpec([]),
            'result' => new TopNewsResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TopNewsMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TopNewsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new TopNewsError('boom', 'boom');
        $utility->fetcher = function (TopNewsContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TopNewsSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TopNewsMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TopNewsContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TopNewsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TopNewsResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TopNewsContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TopNewsSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TopNewsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TopNewsResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TopNewsSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TopNewsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TopNewsMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TopNewsMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TopNewsResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TopNewsDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TopNewsResult(['ok' => false])]);
        $this->expectException(TopNewsError::class);
        TopNewsDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TopNewsResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TopNewsMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        TopNewsMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TopNewsBaseFeature();
        $b = new TopNewsBaseFeature();
        TopNewsFeatureAdd::call($ctx, $a);
        TopNewsFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TopNewsBaseFeature
    {
        $f = new TopNewsBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        TopNewsFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): TopNewsContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TopNewsSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TopNewsPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new TopNewsResponse(['headers' => null]),
            'result' => new TopNewsResult([]),
        ]);
        TopNewsResultHeaders::call($ctx);
