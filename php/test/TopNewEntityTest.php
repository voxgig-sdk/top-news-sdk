...
class TopNewEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = TopNewsSDK::test(null, null);
        $ent = $testsdk->TopNew(null);
    ...
    public function test_stream(): void
    {
        ...
        $base = TopNewsSDK::test($seed, null);
        $seen = iterator_to_array($base->TopNew(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = TopNewsConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = TopNewsSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->TopNew(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = top_new_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "top_new." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $top_new_ref01_data = null;
        if (count($top_new_ref01_data_raw) > 0) {
...
function top_new_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/top_new/TopNewTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["top_new01", "top_new02", "top_new03"] as $k) {
