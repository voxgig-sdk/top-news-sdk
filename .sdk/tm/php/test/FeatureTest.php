...
class FtCtrl extends TopNewsControl
{
...
class FtClient
{
    ...
    private TopNewsUtility $utility;

    public function __construct(TopNewsUtility $utility, array $options)
    {
    ...
    public function get_utility(): TopNewsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TopNewsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TopNewsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TopNewsConfig::make_feature($name);
    ...
    private static function buildUrl(TopNewsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TopNewsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TopNewsResponse($fetched);
        }
    ...
    private function fail(TopNewsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TopNewsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
