...
module TopNewsUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = TopNewsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TopNewsResponse.new(fetched)
    else
