...
class TestTopNewDirect:

    def test_should_direct_list_top_new(self):
        setup = _top_new_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-top_new", "live" if setup["live"] else "unit")
        if _skip:
...
def _top_new_direct_setup(mockres):
