...
class TestTopNewEntity:

    def test_should_create_instance(self):
        testsdk = TopNewsSDK.test(None, None)
        ent = testsdk.TopNew(None)
    ...
    def test_should_stream(self):
        ...
        base = TopNewsSDK.test(seed, None)
        seen = list(base.TopNew(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.TopNew(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _top_new_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "top_new." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set TOP_NEWS_TEST_TOP_NEW_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        top_new_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.top_new")))
        top_new_ref01_data = None
        if len(top_new_ref01_data_raw) > 0:
...
def _top_new_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/top_new/TopNewTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("TOP_NEWS_TEST_TOP_NEW_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("TOP_NEWS_TEST_LIVE") == "TRUE":
        ...
        client = TopNewsSDK(helpers.to_map(merged_opts))

    _live = env.get("TOP_NEWS_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("TOP_NEWS_TEST_EXPLAIN") == "TRUE",
        "live": _live,
