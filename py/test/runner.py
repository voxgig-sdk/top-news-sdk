...
class TopNewsTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            TopNewsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = TopNewsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = TopNewsTestRunner.getenv("TOP_NEWS_TEST_LIVE")
        override = TopNewsTestRunner.getenv("TOP_NEWS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = TopNewsTestRunner.getenv(key)
        ...
        explain = TopNewsTestRunner.getenv("TOP_NEWS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["TOP_NEWS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if TopNewsTestRunner._test_control is not None:
            return TopNewsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                TopNewsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            TopNewsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = TopNewsTestRunner.load_test_control()
...
def load_env_local():
    TopNewsTestRunner.load_env_local()


def env_override(m):
    return TopNewsTestRunner.env_override(m)


def entity_data(v):
    return TopNewsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return TopNewsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return TopNewsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return TopNewsTestRunner.load_test_control()


def live_client_options():
    return TopNewsTestRunner.live_client_options()


def live_delay_ms():
    return TopNewsTestRunner.live_delay_ms()
