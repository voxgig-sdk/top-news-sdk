...
from topnews_sdk.topnews_types import (
    TopNew,
    TopNewListMatch,
)
...
class TopNewEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TopNewEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[TopNew]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.TopNew().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
