...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.TOPNEWS_TEST_LIVE ||
    'TRUE' === process.env.TOPNEWS_TEST_OVERRIDE
  ) {
  ...
  m.TOPNEWS_TEST_EXPLAIN = process.env.TOPNEWS_TEST_EXPLAIN || m.TOPNEWS_TEST_EXPLAIN
