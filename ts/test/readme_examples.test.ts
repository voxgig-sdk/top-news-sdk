...
//     (\`new TopNewsSDK(...)\` / \`TopNewsSDK.test(...)\` become
//     \`TopNewsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"top_new":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TopNewsSDK(...)\` and any
// \`TopNewsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TopNewsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TopNewsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
